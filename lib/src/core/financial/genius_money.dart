...
///
/// All arithmetic is exact integer arithmetic except [multiplyByRate], which
/// rounds exactly once using the provided [GeniusRoundingPolicy].
///
/// ## Example
/// ```dart
...
class GeniusMoney {
  ...
  factory GeniusMoney.zero({String currency = 'SAR', int decimalPlaces = 2}) =>
      GeniusMoney.fromMinorUnits(0, currency: currency, decimalPlaces: decimalPlaces);
  ...
  bool isWithinTolerance(GeniusMoney other, GeniusRoundingPolicy policy) {
    ...
    final factor = pow(10, decimalPlaces);
    // Convert absolute tolerance to integer minor units (default = 1 minor unit).
