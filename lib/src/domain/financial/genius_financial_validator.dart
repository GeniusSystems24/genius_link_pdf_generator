...
/// Stateless financial validator. Constructed with a [GeniusRoundingPolicy];
...
class GeniusFinancialValidator {
  ...
  GeniusMoney _m(double value) =>
      GeniusMoney.fromDouble(value, policy: policy);
  ...
  /// [vatBase] must already be the post-discount base (subtotal − discounts).
  ...
  GeniusFinancialValidationResult validateGrandTotal({
    ...
    final expected =
        _m(subtotal) - _m(discounts) + _m(vatAmount) + _m(fees);
  ...
  /// (or [policy] if null), then validates [sourceAmount] × [exchangeRate] ≈
  /// [providedTargetAmount] using [policy].
  ...
  /// Validates that sum(debits) == sum(credits) exactly (strict integer equality).
  /// Double-entry bookkeeping requires an exact balance — no tolerance is applied.
  GeniusFinancialValidationResult validateAccountingEntries({
    ...
    if (debitM.minorUnits == creditM.minorUnits) return GeniusFinancialValidationResult.valid();
    return GeniusFinancialValidationResult.invalid([
      GeniusFinancialValidationError(
  ...
  /// Guards against division by zero when budget is 0.
  ...
  /// Pre-rounds [rawAmount] to the safe value to pass to AmountToWords.
