...
final areaChart = GeniusPdfAreaChart(
  ...
  Future<void> _generateBarChart(GeniusBarChartType type) async {
    ...
    try {
      ...
      _drawTitle(page, 'Bar Chart - ${type.name.toUpperCase()}');
      ...
      await _saveAndOpenPdf(document, 'bar_chart_${type.name}');
    } catch (e) {
  ...
  Future<void> _generateLineChart(GeniusLineChartType type,
    ...
    try {
      ...
      final boldFont = PdfTrueTypeFont(
          (geniusPdfConfig.assets.secondaryFont ??
                  geniusPdfConfig.assets.primaryFont)
              .toList(),
          10);
      ...
      await _saveAndOpenPdf(
          document, 'line_chart_${type.name}${fillArea ? '_filled' : ''}');
    } catch (e) {
      _showError(e.toString());
    } finally {
      setState(() => _isGenerating = false);
  ...
  Future<void> _generatePieChart({
    ...
    try {
      ...
      _drawTitle(page, isDonut ? 'Donut Chart' : 'Pie Chart');
      ...
      await _saveAndOpenPdf(document, 'pie_chart');
    } catch (e) {
      _showError(e.toString());
    } finally {
      setState(() => _isGenerating = false);
  ...
  Future<void> _generateAreaChart({
    ...
    try {
      ...
      chart.draw(page, Rect.fromLTWH(20, 80, pageSize.width - 40, 300));
      await _saveAndOpenPdf(document, 'area_chart');
    } catch (e) {
      _showError(e.toString());
    } finally {
      setState(() => _isGenerating = false);
  ...
  // --- Helper Methods ---
  void _drawTitle(PdfPage page, String text) {
    final titleFont =
