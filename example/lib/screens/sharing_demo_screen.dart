...
class SharingDemoScreen extends StatefulWidget {
  const SharingDemoScreen({super.key});
...
class _SharingDemoScreenState extends State<SharingDemoScreen>
  ...
  late GeniusShareService _shareService;
  late GeniusEmailShareService _emailService;
  late GeniusBluetoothShareService _bluetoothService;
  late GeniusAppShareService _appService;

  List<GeniusShareHistoryItem> _history = [];
  List<GeniusQuickShareContact> _contacts = [];
  ...
  void initState() {
    ...
    _shareService = GeniusShareService.instance;
    _emailService = GeniusEmailShareService.instance;
    _bluetoothService = GeniusBluetoothShareService.instance;
    _appService = GeniusAppShareService.instance;
    _initializeServices();
    _generateSamplePdf();
  }
  ...
  Future<void> _generateSamplePdf() async {
    ...
    try {
      ...
      setState(() {
        _isLoading = false;
        _status = 'Sample PDF ready (${_samplePdfBytes!.length} bytes)';
      });
    } catch (e) {
      setState(() {
        _isLoading = false;
    ...
    return ListView(
      ...
      children: [
        ...
        if (_contacts.isNotEmpty) ...[
          ...
          const SizedBox(height: 8),
          ..._contacts.map((contact) => _buildContactCard(isDark, contact)),
        ],
        const SizedBox(height: 16),
  ...
  Widget _buildBluetoothTab(bool isDark) {
    return ListView(
      ...
      children: [
        ...
        if (_bluetoothService.savedDevices.isNotEmpty) ...[
          const SizedBox(height: 24),
    ...
    return ListView(
      ...
      children: [
        ...
        if (_history.isEmpty)
          ...
          const SizedBox(height: 8),
          ..._history.map((item) => _buildHistoryCard(isDark, item)),
        ],
      ],
    );
  }
  ...
  Widget _buildAppCard(bool isDark, String name, IconData icon,
    ...
    return Container(
      ...
      child: Material(
        color: Colors.transparent,
        child: InkWell(
          onTap: _samplePdfBytes != null && !_isLoading ? onPressed : null,
          borderRadius: BorderRadius.circular(12),
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Column(
  ...
  Future<void> _shareViaSystem() async {
    ...
    setState(() {
      ...
      _status = result.isSuccess
          ? 'Shared successfully!'
          : 'Share ${result.isCancelled ? "cancelled" : "failed"}: ${result.error ?? ""}';
      _history = _shareService.history;
    });
  }
  ...
  Future<void> _shareViaEmail() async {
    ...
    setState(() {
      _isLoading = false;
      _status = result.isSuccess ? 'Email prepared!' : 'Error: ${result.error}';
      _history = _shareService.history;
    });
  }
  ...
  Future<void> _shareViaBluetooth() async {
    ...
    setState(() {
      _isLoading = true;
      _status = 'Preparing Bluetooth share...';
    });
    ...
    setState(() {
      _isLoading = false;
      _status = result.isSuccess ? 'Ready to share!' : 'Error: ${result.error}';
      _history = _shareService.history;
    });
  }
  ...
  Future<void> _composeEmail() async {
    ...
    setState(() {
      _status =
          result.success ? 'Email client opened!' : 'Error: ${result.error}';
    });
  }
  ...
  Future<void> _shareViaNearby() async {
    ...
    setState(() {
      _isLoading = false;
      _status = result.isSuccess ? 'Shared!' : 'Error: ${result.error}';
    });
  }
  ...
  Future<void> _sendToDevice(GeniusBluetoothDevice device) async {
    if (_samplePdfBytes == null) return;
    setState(() {
      _isLoading = true;
      _status = 'Sending to ${device.name}...';
    });
  ...
  Future<void> _shareToApp(GeniusSharableApp app) async {
    if (_samplePdfBytes == null) return;
    setState(() {
      _isLoading = true;
      _status = 'Sharing to ${app.name}...';
    });
  ...
  Future<void> _openInExternalApp() async {
    ...
    setState(() {
      _isLoading = false;
      _status = result.success ? 'Opened!' : 'Error: ${result.error}';
    });
  }
  ...
  Future<void> _quickShareToContact(GeniusQuickShareContact contact) async {
    if (_samplePdfBytes == null) return;
    setState(() {
      _isLoading = true;
      _status = 'Sharing to ${contact.name}...';
    });
  ...
  Future<void> _shareWithTemplate(GeniusShareMessageTemplate template) async {
    if (_samplePdfBytes == null) return;
    setState(() {
      _isLoading = true;
      _status = 'Preparing email...';
